...
if is_llm_available():
    ...
    st.caption(
        "Powered by OpenAI GPT. Suggestions based on your computed farm data — not generic advice."
        if lang=="en"
        else "OpenAI GPT द्वारा संचालित। आपके गणना किए गए खेत डेटा पर आधारित सुझाव।"
    )
    with st.spinner("Generating AI advisory..." if lang=="en" else "AI सलाह तैयार हो रही है..."):
        llm_output = enrich_advisory(
            result=result,
    ...
    if llm_output:
        ...
        st.info("AI advisory temporarily unavailable. Engine recommendations above are complete."
                if lang=="en"
