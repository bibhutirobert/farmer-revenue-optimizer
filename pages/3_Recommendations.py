...
Fixes: session_state LLM caching, Noto font check, translate (not regenerate)
Hindi advisory, markdown stripping for PDF, numbered point formatting.
"""
...
# ── LLM Advisory (V4 — cached, translated, markdown-stripped) ─────────────────
import os, re
...
if is_llm_available():
    ...
    if not _llm_cache["done"]:
        with st.spinner("Generating AI advisory..." if lang=="en"
            ...
            # Step 1: Generate English advisory
            ...
            # Step 2: Translate English → Hindi (same content, not regenerated)
            if _llm_cache["en"] and _check_hindi_font():
                _llm_cache["hi"] = translate_advisory(_llm_cache["en"]) or ""
