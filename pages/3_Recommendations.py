"""
Page 3 — Recommendations (V3)
Shows price source + timestamp, LLM advisory section, and logs usage event.
"""
...
# ── LLM Advisory (V3 — shown only if OpenAI configured) ──────────────────────
# Collect both English and Hindi outputs here so they can be passed to PDF
llm_advisory_en = ""
llm_advisory_hi = ""
...
if is_llm_available():
    ...
    with st.spinner("Generating AI advisory..." if lang=="en" else "AI सलाह तैयार हो रही है..."):
        # Always fetch English (goes into PDF English section)
        llm_advisory_en = enrich_advisory(
            result=result,
        ...
        # Fetch Hindi only if DejaVu font is present (Hindi PDF section active)
        import os
        _fonts_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "fonts")
        _has_font  = os.path.exists(os.path.join(_fonts_dir, "DejaVuSans.ttf"))
        if _has_font:
            llm_advisory_hi = enrich_advisory(
                result=result,
                soil_name=result.soil_name_en or "Alluvial",
                climate_zone=result.climate_zone or "Tropical",
                state=farm_input.state,
    ...
    # Display the advisory in the UI (show whichever language the user selected)
